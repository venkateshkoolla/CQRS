...
{
    public class CompleteProgramsHandlerTests
    {
        private readonly IPrincipal _user;
        private readonly IUserAuthorization _userAuthorization;
        private readonly ILogger<CompleteProgramsHandler> _logger;

        public CompleteProgramsHandlerTests()
        {
            _userAuthorization = Mock.Of<IUserAuthorization>();
            _user = Mock.Of<IPrincipal>();
            _logger = Mock.Of<ILogger<CompleteProgramsHandler>>();
        }

        [Fact]
        [UnitTest("Handlers")]
        public void CompletePrograms_ShouldPass()
        {
            // Arrange
            ...
            var request = new CompletePrograms
            {
                User = _user,
            ...
            domesticContextMock.Setup(m => m.HasProgramChoices(It.IsAny<Guid>())).ReturnsAsync(true);
            domesticContextMock.Setup(m => m.GetApplication(It.IsAny<Guid>())).ReturnsAsync(application);
            var handler = new CompleteProgramsHandler(_logger, domesticContextMock.Object, _userAuthorization);
            ...
            application.CompletedSteps.Should().Be((int)ApplicationCompletedSteps.ProgramChoice);
        }

        [Fact]
        [UnitTest("Handlers")]
        public void CompletePrograms_ShouldPass_WhenAlreadyCompleted()
        {
            // Arrange
            var programChoices = new List<Dto.ProgramChoice>
            {
        ...
        [Fact]
        [UnitTest("Handlers")]
        public void CompletePrograms_ShouldThrow_WhenNoChoices()
        {
            // Arrange
            var programChoices = new List<Dto.ProgramChoice>();
            var application = new Dto.Application
            ...
            // Assert
            func.Should().Throw<ValidationException>().WithMessage("Must have program choices");
            domesticContextMock.Verify(e => e.UpdateApplication(It.IsAny<Dto.Application>()), Times.Never);
        }

        [Fact]
        [UnitTest("Handlers")]
        public void CompletePrograms_ShouldThrow_WhenNoApplications()
        {
            // Arrange
            var programChoices = new List<Dto.ProgramChoice>();
            var request = new CompletePrograms
