...
namespace Ocas.Domestic.Apply.Api.Services.Extensions
{
    ...
    {
        public static async Task<Guid> GetSourceId(this ILookupsCache lookupsCache, string partnerCode)
        {
        ...
        public static async Task<(Guid, Guid?)> GetSourcePartnerId(this ILookupsCache lookupsCache, string partnerCode)
        {
            var sources = await lookupsCache.GetSources(Constants.Localization.FallbackLocalization);
            ...
            if (!string.IsNullOrEmpty(partnerCode))
            {
                ...
                if (sourcePartnerCollege != null)
                {
                    ...
                    {
                        var sourceCode = Constants.Sources.CBUI + sourcePartnerCollege.Code;
                        sourceId = sources.FirstOrDefault(s => s.Code == sourceCode)?.Id ?? sources.First(s => s.Code == Constants.Sources.CBUIUNKNOWN).Id;
                    }
                    else
                    {
                        sourceId = sources.First(x => x.Code == Constants.Sources.CBUI).Id;
                    }

                    return (sourceId, sourcePartnerCollege.Id);
                }
                ...
                if (sourcePartnerReferral != null)
                {
                    ...
                    return (sourceId, sourcePartnerReferral.Id);
                }
            }
